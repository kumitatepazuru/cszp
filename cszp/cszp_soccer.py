...
def loop(cmd, loops, lang, module):
    subp.reset()
    inp = ""
    ...
    for _ in tqdmd:
        ...
        if i3 == "y":
            ...
            temp.close()
            cszp_html.plot_socre(file, module)

            data = module.Open("./config/config.conf")
            path = data.read().split(",")[9]
            data.close()
            temp = open(path + "/data.csv", "a")
            soccer = cszp_log.log(logs)
            temp.write(
                "\n" + exittime + "," + soccer[0] + "," + soccer[1] + "," + soccer[2] + "," + soccer[3] + "," + str(
                    int(soccer[2]) - int(soccer[3])) + "," + str(int(soccer[3]) - int(soccer[2])))
...
def start(cmd, lang, module):
    ...
    try:
        ...
        pass
    soccer = cszp_log.log(logs)
    print(soccer[0] + " " + soccer[2] + " - " + soccer[3] + " " + soccer[1] + " " + cszp_log.kekka(logs, 0, module))
    inp = ""
    while inp.lower() != "y" and inp.lower() != "n":
        inp = subp.Input(lang.lang("プログラムログを保存しますか？ Y/n"))
        if inp.lower() != "y" and inp.lower() != "n":
...
def setting(lang, module, testmode=False, loopmode=False):
    ok = 0
    cmd = []
    ...
    while ok == 0:
        ...
        print("\033[0m")
        inp = subp.Inputfilegui(lang.lang("Team1(黄色)チームのパスを入力(リストの名前を入力したい場合はQキー)"))
        if inp == -1:
            print("\033[0m\033[38;5;172m")
            v = open("./version")
            subprocess.check_call("figlet -ctk cszp " + v.read(), shell=True)
            ...
            inp = subp.Input(lang.lang("\nTeam1(黄色)チームのリストの名前を入力"), textcolor="\033[38;5;11m")
        if inp == -2:
    ...
    while ok == 0 and ok != 2:
        ...
        print("\033[0m")
        inp = subp.Inputfilegui(lang.lang("Team2(赤色)チームのパスを入力(リストの名前を入力したい場合はQキー)"))
        if inp == -1:
            print("\033[0m\033[38;5;172m")
            v = open("./version")
            subprocess.check_call("figlet -ctk cszp " + v.read(), shell=True)
            ...
            print("\n\033[38;5;39m" + lang.lang("簡単サッカー実行リスト"))
            print(
                table.draw() + lang.lang("\n前画面に戻る場合はbackと入力") + "\n\033[38;5;11m" + lang.lang("Team1(黄色)チームのパス:") +
                cmd[0])
            inp = subp.Input(lang.lang("\nTeam2(赤色)チームのリストの名前を入力"), textcolor="\033[38;5;9m")
        if inp == -2:
            raise KeyboardInterrupt
